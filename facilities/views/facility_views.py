...
from common.models import ContactType, UserConstituency, UserCounty, UserSubCounty
...
class QuerysetFilterMixin(object):
    ...
    def filter_for_county_users(self):
        if (
            not self.request.user.is_national
            and self.request.user.county
            and "ward"
            in [field.name for field in self.queryset.model._meta.get_fields()]
        ):
            try:
                self.queryset = self.queryset.filter(
                    ward__constituency__county__in=[
                        uc.county.id
                        for uc in UserCounty.objects.filter(
    ...
    def filter_for_sub_county_users(self):
        if (
            ...
            and "ward"
            in [field.name for field in self.queryset.model._meta.get_fields()]
            and not self.request.user.constituency
        ):
            try:
                self.queryset = self.queryset.filter(
    ...
    def filter_for_consituency_users(self):
        if (
            ...
            and not self.request.user.sub_county
        ):
            try:
                self.queryset = self.queryset.filter(
    ...
    def filter_for_sub_county_and_constituency_users(self):
        if (
            ...
            and "ward"
            in [field.name for field in self.queryset.model._meta.get_fields()]
            and self.request.user.constituency
        ):
            try:
                self.queryset = self.queryset.filter(
                    ward__sub_county__in=[
    ...
    def filter_classified_facilities(self):
        if self.request.user.has_perm(
            ...
            field.name for field in self.queryset.model._meta.get_fields()
        ]:
    ...
    def filter_approved_facilities(self):
        if self.request.user.has_perm(
            ...
            try:
                self.queryset = self.queryset.filter(
                    approved=True, operation_status__is_public_visible=True
                )
            except:
                self.queryset = self.queryset.filter(
    ...
    def filter_rejected_facilities(self):
        if self.request.user.has_perm(
            "facilities.view_rejected_facilities"
        ) is False and (
            "rejected"
            in [field.name for field in self.queryset.model._meta.get_fields()]
        ):
    ...
    def filter_closed_facilities(self):
        if self.request.user.has_perm(
            "facilities.view_closed_facilities"
        ) is False and "closed" in [
            field.name for field in self.queryset.model._meta.get_fields()
        ]:
    ...
    def filter_for_infrastructure(self):
        if self.request.user and hasattr(self.request, "infrastruture"):
            ...
            self.queryset = self.queryset.filter(
                infrastructure=self.request.infrastructure
            )

    def filter_for_services(self):
        if self.request.user.has_perm("facilities.view_facilityservice") is False and (
            "service"
            in [field.name for field in self.queryset.model._meta.get_fields()]
        ):
...
class KephLevelDetailView(AuditableDetailViewMixin, CustomRetrieveUpdateDestroyView):
    """
...
class FacilityUnitDetailView(AuditableDetailViewMixin, CustomRetrieveUpdateDestroyView):
    """
...
class OwnerDetailView(AuditableDetailViewMixin, CustomRetrieveUpdateDestroyView):
    """
...
class FacilityExportMaterialListView(QuerysetFilterMixin, generics.ListAPIView):
    queryset = FacilityExportExcelMaterialView.objects.all()  # FacilityExportExcelMaterialView.objects.none() # For disabling facilities material export
...
class FacilityDetailView(
    QuerysetFilterMixin, AuditableDetailViewMixin, generics.RetrieveUpdateDestroyAPIView
):
    """
    ...
    def buffer_contacts(self, update, contacts):
        ...
        if update.contacts and update.contacts != "null":
            ...
            # remove duplicates
            updated_contacts = [
                contact for contact in contacts if contact not in proposed_contacts
            ]
    ...
    def buffer_services(self, update, services):
        ...
        if update.services and update.services != "null":
            ...
            # remove duplicates
            updated_services = [
                service for service in services if service not in proposed_services
            ]
    ...
    def buffer_humanresources(self, update, humanresources):
        ...
        if update.humanresources and update.humanresources != "null":
            ...
            updated_humanresources = [
                hr for hr in humanresources if hr not in proposed_humanresources
            ]
            update.humanresources = json.dumps(
                proposed_humanresources + updated_humanresources
            )
        else:
    ...
    def buffer_infrastructure(self, update, infrastructure):
        ...
        if update.infrastructure and update.infrastructure != "null":
            ...
            updated_infrastructure = [
                ...
                if infra not in proposed_infrastructure
            ]
            update.infrastructure = json.dumps(
                proposed_infrastructure + updated_infrastructure
            )
        else:
    ...
    def buffer_units(self, update, units):
        ...
        if update.units and update.units != "null":
            ...
            # remove duplicates
            updated_units = [unit for unit in units if unit not in proposed_units]
    ...
    def populate_service_name(self, services):
        ...
        for service in services:
            ...
            if service_id in resolved_ids:
                ...
                if option:
                    service["display_name"] = Option.objects.get(id=option).display_text
        return services
    ...
    def populate_humanresources_name(self, humanresources):
        ...
        for hr in humanresources:
            ...
            if humanresources_id in resolved_ids:
                ...
                if option:
                    hr["display_name"] = Option.objects.get(id=option).display_text
    ...
    def should_buffer_officer_incharge(self, officer_in_charge, instance):
        ...
        if not old_details and officer_in_charge != {}:
            ...
            if officer_in_charge.get("title") != str(old_details.get("title")):
                verdict = True
            if officer_in_charge.get("reg_no") != str(old_details.get("reg_no")):
                verdict = True
    ...
    def something_changed(
        ...
        if (
            ...
            and contacts == []
            and units == []
            and not self.should_buffer_officer_incharge(officer_in_charge, instance)
        ):
            return False
        return True
    ...
    def _validate_payload(
        ...
        if officer_errors and officer_in_charge != {} and officer_errors is not True:
            self.validation_errors.update({"officer_in_charge": officer_errors})
    ...
    def update(self, request, *args, **kwargs):
        ...
        officer_in_charge = request.data.pop("officer_in_charge", {})

        officer_in_charge = (
            {} if officer_in_charge.get("name") == "" else officer_in_charge
        )
        ...
        self._validate_payload(
            services, humanresources, infrastructure, contacts, units, officer_in_charge
        )
        if any(self.validation_errors):
            return Response(
                data=self.validation_errors, status=status.HTTP_400_BAD_REQUEST
            )
        if instance.approved and self.something_changed(
            ...
            try:
                update = FacilityUpdates.objects.filter(
                    facility=instance, cancelled=False, approved=False
                )[0]
            except IndexError:
                update = FacilityUpdates.objects.create(
                    facility=instance, created_by=request.user, updated_by=request.user
                )
            ...
            if officer_in_charge != {} and self.should_buffer_officer_incharge(
...
class FacilityOfficerListView(LoginRequiredMixin, generics.ListCreateAPIView):
    ...
    ordering_fields = ("name", "id_number", "registration_number", "facility_name")
...
class OptionGroupDetailView(AuditableDetailViewMixin, CustomRetrieveUpdateDestroyView):
    queryset = OptionGroup.objects.all()
    serializer_class = OptionGroupSerializer
...
class RegulatorSyncListView(generics.ListCreateAPIView):
    ...
    ordering_fields = ("name", "registration_number", "county", "owner", "mfl_code")
...
class SpecialityDetailView(AuditableDetailViewMixin, CustomRetrieveUpdateDestroyView):
    """
